 ...
 <div class="container text-center">
            <ul class="pagination">
            	...
            	<c:when test="${(pageNum-1) >= 0}">
                    <mylib:link target="${target}" body="<span aria-hidden='true'>&laquo;</span>" pageNum="${pageNum-1}" ariaLabel="Previous"/>
                     </c:when>
            	 	<c:otherwise>
            	 	<mylib:link target="${target}" body="<span aria-hidden='true'>&laquo;</span>" pageNum="${pageNum}" ariaLabel="Previous"/>
            	 	</c:otherwise>
            	 </c:choose>
              </li>
              ...
              <c:choose>
            	<c:when test="${(pageNum+1)*range < nb}">
                 <mylib:link target="${target}" body="<span aria-hidden='true'>&raquo;</span>" pageNum="${pageNum+1}" ariaLabel="Next"/>
                   
                </c:when>
            	 	<c:otherwise>
                 <mylib:link target="${target}" body="<span aria-hidden='true'>&raquo;</span>" pageNum="${pageNum}" ariaLabel="Next"/>
            	 	</c:otherwise>
            	 </c:choose>
                </a>
            </li>
